...
const HelpCenter = () => {
	...
	return (
		<SafeAreaView className="flex-1">
			<ScrollView className="bg-background h-full pt-8">
				<View className="flex-row justify-start items-center gap-4 px-4 w-full ">
					<Text className="text-2xl font-semibold font-sans text-popover-foreground pl-3">
						Help Center
					</Text>
				</View>
					...
					{tabNames.map((item, idx) => (
						<Pressable
							...
							className={`justify-center items-center w-1/2 border-b-2   pb-2 ${
								activeTab === idx ? " border-primary" : "border-primary"
							}`}
						>
							<Text
								className={`text-center text-lg ${
									activeTab === idx
										? "text-popover-foreground border-primary"
										: "border-primary text-popover-foreground"
								} font-semibold `}
							>
								{item}
							</Text>
					...
					<View className="pt-8 gap-4 px-6">
						{faqData.map(({ id, question, answer }, idx) => (
							<Pressable
								...
								className={`p-4 rounded-xl border border-border shadow-md  ${
									activeFaq === idx ? "bg-card" : ""
								}`}
								key={id}
							>
								<View className="flex-row justify-between items-start">
									<Text
										className={`font-semibold font-sans text-muted-foreground pr-5 text-g60 ${
											activeFaq === idx ? "" : ""
										}`}
									>
										{question}
								...
								{activeFaq === idx && (
									<Text className="pt-3  border-t border-dashed mt-3  border-border text-muted-foreground ">
										{answer}
									</Text>
								)}
							</Pressable>
				...
				{activeTab === 1 && (
					<View className="p-5 rounded-2xl border border-g30 mx-6 flex flex-col mt-8">
						...
						<View className="flex flex-row justify-start items-center pb-4 border-b border-dashed border-g30  pt-5 gap-2">
							...
							<Text className="text-base font-semibold text-g60">WhatsApp</Text>
						</View>
						<View className="flex flex-row justify-start items-center pb-4 border-b border-dashed border-g30  pt-5 gap-2">
							<MaterialCommunityIcons name="web" size={20} color="#3f3f3f" />
							<Text className="text-base font-semibold text-g60">Website</Text>
						</View>
						<View className="flex flex-row justify-start items-center pb-4 border-b border-dashed border-g30  pt-5 gap-2">
							<FontAwesome name="facebook" size={20} color="#3f3f3f" />
							<Text className="text-base font-semibold text-g60">Facebook</Text>
						</View>
						<View className="flex flex-row justify-start items-center pb-4 border-b border-dashed border-g30  pt-5 gap-2">
							<FontAwesome6 name="x-twitter" size={20} color="#3f3f3f" />
							<Text className="text-base font-semibold text-g60">Twitter</Text>
						</View>
